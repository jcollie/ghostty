...
// On Windows, Zig's _DllMainCRTStartup does no C runtime initialization
// at all: it is the image's entry point, so neither MSVC's CRT bootstrap
// nor MinGW's dllcrt2.obj is ever reached. Declaring DllMain causes Zig's
// start.zig to call us during DLL_PROCESS_ATTACH instead. See
// lib/windows_dll.zig for what has to be done by hand and why.
pub const DllMain = if (builtin.os.tag == .windows)
